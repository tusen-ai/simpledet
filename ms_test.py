...
if __name__ == "__main__":
    ...
    for index_split in range(int(math.ceil(len(roidbs_all) / split_size))):
        ...
        for index in range(loader.total_record):
            ...
            rid, id, info, post_cls_score, post_box, post_cls, mask, iou_pred = rid.squeeze(), id.squeeze(), info.squeeze(), \
                                                                                ...
                                                                                post_cls.squeeze(), mask.squeeze(), iou_pred.squeeze()
            ...
            iou_score = iou_pred[valid_inds, cls + 1].reshape(-1, )
            new_score = cls_score * iou_score
        ...
        for rec in all_outputs:
            ...
            if im_id not in output_dict:
                ...
                output_dict[im_id]["new_score"].append(rec["new_score"])
            ...
            output_dict[im_id]["new_score"] = output_dict[im_id]["new_score"][0]
        ...
        for k in output_dict:
            ...
            new_score = output_dict[k]["new_score"]
            ...
            for cid in np.unique(cls):
                ...
                new_score_of_this_class = new_score[ind_of_this_class]
            ...
            output_dict[k]["new_score"] = final_new_scores
        ...
        for iid in output_dict:
            ...
            for cid in output_dict[iid]["det_xyxys"]:
                ...
                new_scores = output_dict[iid]["new_score"][cid]
    ...
    for ii in range(len(coco_result)):
        coco_result[ii]['score'] = coco_result[ii]['new_score']

    # after multiply mask scoring
    ann_type = 'segm'
    coco_dt = coco.loadRes(coco_result)
    coco_eval = COCOeval(coco, coco_dt)
    coco_eval.params.useSegm = (ann_type == 'segm')
