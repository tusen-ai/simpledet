...
def SEPCFPN(inputs,
                 ...
                 start_level=1,
                 norm=None,
                 bilinear_upsample=None,
                 feat_sizes=None,
                 ):
    assert feat_sizes is not None
    Pconvs_list = []
    print('lcconv_deform: {} bilinear_upsample: {} '.format(lcconv_deform, bilinear_upsample))

    for i in range(Pconv_num):
        Pconvs_list.append(partial(PConvModule, out_channels=out_channels, iBN=iBN, part_deform=pconv_deform, 
    ...
    if lcconv_deform is not None:
        ...
        if iBN is not None:
            assert norm is not None and iBN in [False, True]
            if iBN is False:
...
def PConvModule(x,out_channels=256,
                 kernel_size=[3, 3, 3],
                 dilation=[1, 1, 1],
                 groups=[1, 1, 1],
                 iBN=None,
    ...
    sepc0_weight=X.var(name=name_pref+'0_weight',init=X.gauss(std=0.01))
    sepc0_bias=X.var(name=name_pref+'0_bias',init=X.zero_init())
    sepc1_weight=X.var(name=name_pref+'1_weight',init=X.gauss(std=0.01))
    sepc1_bias=X.var(name=name_pref+'1_bias',init=X.zero_init())
    sepc2_weight=X.var(name=name_pref+'2_weight',init=X.gauss(std=0.01))
    sepc2_bias=X.var(name=name_pref+'2_bias',init=X.zero_init())
    ...
    if iBN is not None:
        ...
        if iBN is False:
            ...
            norm_func.append(partial(norm, name=name_pref+'_iBN'))

    sepc_conv0_func=partial(sepc_conv, name='PConv{}_sepc0_'.format(PConv_idx),out_channels=out_channels,
    ...
    sepc_conv1_func=partial(sepc_conv, name='PConv{}_sepc1_'.format(PConv_idx),out_channels=out_channels,
    ...
    sepc_conv2_func=partial(sepc_conv, name='PConv{}_sepc2_'.format(PConv_idx),out_channels=out_channels,
                ...
                weight=sepc2_weight,bias=sepc2_bias,weight_offset=sepc2_offset_weight,bias_offset=sepc2_offset_bias)  # NOTE stride
    ...
    for level, feature in enumerate(x):
        ...
        if level < len(x) - 1:
            ...
            if bilinear_upsample is True:
                tmp_x = mx.contrib.symbol.BilinearResize2D(tmp_x, scale_height=2, scale_width=2,
                    name='PConv{}_upsampling_level{}'.format(PConv_idx,level))  # height=_Null, width=_Null, like=None, mode=_Null, align_corners=_Null, 
            else:
                tmp_x = mx.sym.UpSampling(tmp_x,scale=2,sample_type='nearest',num_args=1,
                    name='PConv{}_upsampling_level{}'.format(PConv_idx,level))  # NOTE use nearest instead of bilinear
            tmp_x = mx.sym.slice_like(tmp_x, temp_fea)
            temp_fea = temp_fea+tmp_x
        next_x.append(temp_fea)
    if iBN is not None:
        next_x = iBN_func(next_x, norm_func[0], feat_sizes)
    next_x = [relu(item,name='PConv{}_level{}_relu'.format(PConv_idx, level)) for level,item in enumerate(next_x)]   # NOTE NOTE NOTE changed from relu6 to relu
    return next_x
