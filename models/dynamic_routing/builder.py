...
def dr_resnet_unit(input, name, filter, stride, dilate, proj, norm):
    ...
    conv2_1 = conv(relu1, name=name + "_conv2_1", weight=conv2_weight,
        filter=filter // 4, kernel=3, stride=stride, dilate=1)
    conv2_2 = conv(relu1, name=name + "_conv2_2", weight=conv2_weight,
        filter=filter // 4, kernel=3, stride=stride, dilate=2)
    conv2_3 = conv(relu1, name=name + "_conv2_3", weight=conv2_weight,
        filter=filter // 4, kernel=3, stride=stride, dilate=3)
...
def dr_resnet_stage(data, name, num_block, filter, stride, dilate, norm):
    s, d = stride, dilate
    ...
    for i in range(num_block + 1 - 3, num_block + 1):
        data = dr_resnet_unit(data, "{}_unit{}".format(name, i), filter, 1, d, False, norm)
