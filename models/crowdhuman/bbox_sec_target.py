...
def _sample_proposal(proposals, gt_bboxes, image_rois, fg_fraction, fg_thresh, bg_thresh_hi,
    ...
    """
    ignore_label = -2
    ignore_gt_index = np.where(gt_bboxes[:, 4] == ignore_label)[0]
    gt_bboxes[ignore_gt_index, 4] = -1

    num_proposal = proposals.shape[0]
    max_assign_class = np.zeros((num_proposal,))
    max_assign_iou = np.zeros((num_proposal,))
    max_assign_index = np.zeros((num_proposal, ))
    sec_max_assign_iou = np.zeros((num_proposal, ))
    sec_max_assign_index = np.zeros((num_proposal, ))

    for cid in range(1, num_reg_class):
        gt_index = np.where(gt_bboxes[:, 4] == cid)[0]
        select_gt_bboxes = gt_bboxes[gt_index]
        if select_gt_bboxes.shape[0] == 0:
            continue
        proposal_to_gt_overlaps = bbox_overlaps(
            proposals.astype(np.float32, copy=False),
            select_gt_bboxes.astype(np.float32, copy=False)
        )
        # find max iou GT
        ...
        # find sec max iou GT
        proposal_to_gt_overlaps[np.arange(num_proposal), proposal_assigned_gt_index] = -1
        proposal_sec_max_overlap_w_gt = proposal_to_gt_overlaps.max(axis=1)
        proposal_sec_assigned_gt_index = proposal_to_gt_overlaps.argmax(axis=1)

        # find valid index in this class
        valid_index = np.where(proposal_max_overlap_w_gt > max_assign_iou)[0]
        max_assign_class[valid_index] = cid
        max_assign_iou[valid_index] = proposal_max_overlap_w_gt[valid_index]
        max_assign_index[valid_index] = gt_index[proposal_assigned_gt_index[valid_index]]       # NOTE relative pos in GT_BBOX
        sec_max_assign_index[valid_index] = gt_index[proposal_sec_assigned_gt_index[valid_index]]   # NOTE relative pos in GT_BBOX
        sec_max_assign_iou[valid_index] = proposal_sec_max_overlap_w_gt[valid_index]

    proposal_max_overlap_w_gt = max_assign_iou
    proposal_assigned_class = max_assign_class
    proposal_assigned_gt_index = max_assign_index.astype(np.int64)
    proposal_sec_max_overlap_w_gt = sec_max_assign_iou
    proposal_sec_assigned_gt_index = sec_max_assign_index.astype(np.int64)
    ...
    # sec label
    fg_proposal_to_gt_overlaps_sec = proposal_sec_max_overlap_w_gt[keep_inds]
    bg_sec_labels = np.where(fg_proposal_to_gt_overlaps_sec < fg_thresh)[0]
    sampled_sec_labels = proposal_assigned_class[keep_inds]
    # sampled_sec_labels[fg_rois_per_image:] = 0
    sampled_sec_labels[bg_sec_labels] = 0
    sampled_sec_gt_bboxes = gt_bboxes[proposal_sec_assigned_gt_index[keep_inds]]

    bbox_targets = bbox_transform_inv(sampled_proposals, sampled_gt_bboxes, inv_stds)
    bbox_sec_targets = bbox_transform_inv(sampled_proposals, sampled_sec_gt_bboxes, inv_stds)

    bbox_class = sampled_labels[:, None]
    bbox_sec_class = sampled_sec_labels[:, None]

    if num_reg_class == 2:
        bbox_class = np.array(bbox_class > 0, dtype=bbox_targets.dtype)
        bbox_sec_class = np.array(bbox_sec_class > 0, dtype=bbox_targets.dtype)
    bbox_targets_with_class = np.concatenate([bbox_class, bbox_targets], axis=1)
    bbox_sec_targets_with_class = np.concatenate([bbox_sec_class, bbox_sec_targets], axis=1)
    bbox_targets, bbox_weights = _expand_bbox_targets(bbox_targets_with_class, num_reg_class)
    bbox_sec_targets, bbox_sec_weights = _expand_bbox_targets(bbox_sec_targets_with_class, num_reg_class)

    return sampled_proposals, sampled_labels, bbox_targets, bbox_weights, sampled_sec_labels, bbox_sec_targets, bbox_sec_weights
...
class BboxTargetOperator(mx.operator.CustomOp):
    ...
    def forward(self, is_train, req, in_data, out_data, aux):
        ...
        sampled_proposal, bbox_class, bbox_target, bbox_target_weight, bbox_sec_class, bbox_sec_target, bbox_sec_target_weight = [], [], [], [], [], [], []
        for i in range(batch_image):
            ...
            sampled_proposal_i, bbox_class_i, bbox_target_i, bbox_target_weight_i, bbox_sec_class_i, bbox_sec_target_i, bbox_sec_target_weight_i = output
            ...
            bbox_sec_class.append(bbox_sec_class_i)
            bbox_sec_target.append(bbox_sec_target_i)
            bbox_sec_target_weight.append(bbox_sec_target_weight_i)
        ...
        bbox_sec_class = np.array(bbox_sec_class, dtype=np.float32)
        bbox_sec_target = np.array(bbox_sec_target, dtype=np.float32)
        bbox_sec_target_weight = np.array(bbox_sec_target_weight, dtype=np.float32)

        for i, val in enumerate([sampled_proposal, bbox_class, bbox_target, bbox_target_weight,\
                                     bbox_sec_class, bbox_sec_target, bbox_sec_target_weight]):
...
@mx.operator.register('doublebbox_target')
class BboxTargetProp(mx.operator.CustomOpProp):
    ...
    def list_outputs(self):
        return ['sampled_proposal', 'bbox_cls', 'bbox_target', 'bbox_target_weight', 'bbox_sec_cls', 'bbox_sec_target', 'bbox_sec_target_weight']
    ...
    def infer_shape(self, in_shape):
        ...
        return [rpn_rois_shape, gt_boxes_shape], \
               [sampled_proposal_shape, bbox_cls_shape, bbox_target_shape, bbox_weight_shape, bbox_cls_shape, bbox_target_shape, bbox_weight_shape]
