...
from models.TSD.faster_rcnn import FasterRcnn_TSD as Detector
...
def get_config(is_train):
    ...
    class BboxParam:
        ...
        class TSD:
            ...
            # progressive constraint will harm the overall performance
            pc_cls = False
            pc_reg = False
            pc_cls_margin = 0.2
            pc_reg_margin = 0.2
    ...
    delta_c_pool = FPNRoIAlign_DeltaC(RoiParam) 
    delta_r_pool = FPNRoIAlign_DeltaR(RoiParam) 
    bbox_head = BboxHead(BboxParam, delta_c_pool, delta_r_pool)
    detector = Detector()
    
    from shape_tool import enter_shape_infer_context
    ...
    from models.TSD.metric import LossMetric
    ...
    # for bbox, the label is generated in network so it is an output
    tsd_acc_metric = metric.AccWithIgnore(
        "TSD_Acc",
        ["tsd_bbox_cls_loss_output", "bbox_label_blockgrad_output"],
        []
    )
    tsd_l1_metric = metric.L1(
        "TSD_L1",
        ["tsd_bbox_reg_loss_output", "bbox_label_blockgrad_output"],
        []
    )
    cls_pc_metric = LossMetric(
        "PC_cls",
        ["cls_pc_loss_output"],
        []
    )
    reg_pc_metric = metric.L1(
        "PC_reg",
        ["reg_pc_loss_output", "bbox_label_blockgrad_output"],
        []
    )
    metric_list = [rpn_acc_metric, rpn_l1_metric, box_acc_metric, box_l1_metric, tsd_acc_metric, tsd_l1_metric, cls_pc_metric, reg_pc_metric]
