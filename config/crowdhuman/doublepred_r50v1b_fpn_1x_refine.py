...
from models.crowdhuman.builder import DoublePredFPNRpnHead as RpnHead
from models.crowdhuman.builder import DoublePredRcnn as Detector
from models.crowdhuman.builder import DoublePredFPNBbox2fcHead as BboxHead
...
def get_config(is_train):
    ...
    class OptimizeParam:
        ...
        class schedule:
            begin_epoch = 0
            end_epoch = 10
            lr_iter = [14960 * 16 // (len(KvstoreParam.gpus) * KvstoreParam.batch_image),
                       17765 * 16 // (len(KvstoreParam.gpus) * KvstoreParam.batch_image)]
    ...
    emd_metric = metric.ScalarLoss(
        "emd",
        ['cls_reg_loss_output', ],
        []
    )

    refine_emd_metric = metric.ScalarLoss(
        "refine_emd",
        ['refine_cls_reg_loss_output', ],
        []
    )

    metric_list = [rpn_acc_metric, rpn_l1_metric, emd_metric, refine_emd_metric]
